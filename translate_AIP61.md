...
在本AIP中，我们阐述了一种对用户更加友好的账户管理途径，该方法基于未经修改的[^openpubkey]**OpenID Connect (OIDC)**标准，并结合了关于**OIDC签名**的**零知识证明(ZKPoKs)_**的最新进展[^snark-jwt-verify]$^,$[^nozee]$^,$[^bonsay-pay]$^,$[^zk-blind]$^,$[^zklogin]。
